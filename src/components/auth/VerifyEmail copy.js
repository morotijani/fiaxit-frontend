...
function VerifyEmail() {
    ...
    async function verifyUser(userId, verifyCode) {
        ...
        try {
            ...
            let errorData = null;
            try {
                // if err has response text (from jsonGet error), parse it
                if (err.message && err.message.startsWith('HTTP')) {
                    // jsonGet throws "HTTP 400: ..." format
                    // try to parse the message or fetch details from response if available
                    console.warn('API error response:', err.message);
                }
                // attempt to read response as JSON if available
                if (err.response) {
            ...
            // default error structure if we couldn't parse response
            if (!errorData) {
                errorData = {
                    success: false,
